...
3. **Google Safe Browsing API (server)**: `POST /api/check-url` in `server.ts` proxies requests to Google's Safe Browsing API. Has in-memory cache (1h TTL). API errors fail closed (assume unsafe). When API key is configured, unknown domains are verified — safe ones are **kept** (not replaced), only truly unsafe URLs are replaced.
