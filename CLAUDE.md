...
### URL Safety Layer (`src/lib/urlSafety.ts` + `src/lib/safeBrowsing.ts`)
...
2. **Post-processing (client)**: `sanitizeTravelPlan()` is called in `App.tsx` on every generated/modified travel plan before setting state. It checks every URL field:
   ...
   - Unknown domains → checked via Safe Browsing API (if available)
3. **Google Safe Browsing API (optional, server)**: `POST /api/check-url` in `server.ts` proxies requests to Google's Safe Browsing API. Has in-memory cache (1h TTL). API errors fail closed (assume unsafe).
